...
if ($meta->get_metadata("model_build.enabled")) {
    ...
    #
    # If we have already processed the model don't resubmit
    #
    my $proc_id = $meta->get_metadata("model_build.reconstruction_id");
    ...
    {
	warn "Model build already submitted as $proc_id\n";
	$meta->add_log_entry($0, "Model build already submitted as $proc_id");
    }
    else
    {
	my $username;
