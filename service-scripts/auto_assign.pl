...
my($prot,$sims,$based_on_sims,$based_on_neigh,$assignments,$seq);
...
#    print STDERR &Dumper(["equiv.ids",$pegs,$external_ids]);
    ...
    if (my($best_function, $source, $score) = &Assignments::choose_best_assignment($fig,$parms,$pegs,$external_ids))
    {
	print "$prot\t$source\t$score\t$best_function\n";
    }
}
